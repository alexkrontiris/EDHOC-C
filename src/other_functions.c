...
unsigned char *key_add_headers(unsigned char *key, size_t key_sz, const char *filepath)
{
	...
	const char end_header[] = "-----END PUBLIC KEY-----";

	int base64_buf_sz = 4*(key_sz/3) + 4;
	char *key_base64 = encode((const char *)key, base64_buf_sz + 1);
	
	unsigned char *key_pem = malloc(strlen(begin_header) + strlen(pem_header) + strlen(key_base64) + strlen(end_header) + 1);

	memcpy(key_pem, begin_header, strlen(begin_header));
	memcpy(key_pem + strlen(begin_header), pem_header, strlen(pem_header));
	memcpy(key_pem + strlen(begin_header) + strlen(pem_header), key_base64, strlen(key_base64));
	memcpy(key_pem + strlen(begin_header) +strlen(pem_header) + strlen(key_base64), end_header, strlen(end_header) + 1);

	FILE *out = fopen(filepath, "w+");
	fputs((const char *)key_pem, out);
	fclose(out);
